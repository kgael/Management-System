...
export default function MovementForm({ items, onSubmit }) {
  ...
  useEffect(() => {
    if (!items.find((i) => i.id === f.itemId) && items[0])
      setF((x) => ({ ...x, itemId: items[0].id }));
  }, [items]);
  ...
  return (
    ...
    >
      ...
      <Field
        ...
        type="number"
        value={f.cantidad}
        onChange={(v) => setF({ ...f, cantidad: Number(v) })}
      />
      <Field
        label="Responsable"
        value={f.responsable}
        onChange={(v) => setF({ ...f, responsable: v })}
      />
      ...
      <div className="md:col-span-2">
        <button
          className="rounded-xl bg-pink-600 px-4 py-2 text-white shadow-sm hover:bg-pink-700"
          type="submit"
        >
          Registrar
        </button>
